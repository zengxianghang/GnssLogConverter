...
- Project-specific exception: for `RANGEA -> RANGEB` and `OBSVMA -> RANGEB`, copy the 32-bit `ch-tr-status` / tracking-status word bit-for-bit. Do not translate, normalize, reinterpret, or rebuild any bit, including system/signal/validity/reserved bits, even if vendor documentation assigns different meanings.
...
4. For `RANGEA -> RANGEB` and `OBSVMA -> RANGEB`, include tests proving exact 32-bit tracking-status preservation, including values with high bits set.
